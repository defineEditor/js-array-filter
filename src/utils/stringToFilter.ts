...
// Split by commas that are not inside single or double quotes
    ...
    try {
        ...
        if (rawFilterUpdated.length >= 3) {
            ...
            while (rawConditionCheck !== null) {
                ...
                const connector = rawConditions.replace(nextConditionConnector, "$1");
        ...
        rawConditionChecks.forEach((rawConditionCheck) => {
            ...
            if (caseInsensitiveColNames) {
                const col = columns.find((column) => column.name.toLowerCase() === columnName.toLowerCase());
                if (col) {
                    columnName = col.name;
                }
            }

            if (Object.prototype.hasOwnProperty.call(operatorLabelsInverse, comparator)) {
            ...
            if (value === "null") {
                ...
                if (!isMultipleValue) {
                    ...
                    const list = (value as string).trim().replace(/^\((.*)\)$/, "$1");
                    value = splitCommaRespectingQuotes(list).map((item) => item.replace(/^(['"])(.*)\1$/, "$2"));
                }
            }
